...
export default function Form(props) {
  ...
  function mailClick() {
    let mailList = [];
    text.split(/(?: |\n)/).filter((element) => {
      // both space and new-line are separators
  ...
  function change(event) {
    // onChange passes an event parameter by default
    ...
    <>
      <div className="container mb-3 my-3 border p-3">
        ...
        <div className="mb-3 my-3">
          <textarea
            className={`form-control bg-${props.mode === "light" ? "white" : "dark"} text-${props.mode === "light" ? "dark" : "light"}`}
            id="myBox"
            rows="8"
        ...
        <button disabled={text.length === 0} className={`btn btn-${props.theme} m-2`} onClick={upClick}>
          Convert to Uppercase
        </button>
        <button disabled={text.length === 0} className={`btn btn-${props.theme} m-2`} onClick={lowClick}>
          Convert to Lowercase
        </button>
        <button disabled={text.length === 0} className={`btn btn-${props.theme} m-2`} onClick={mailClick}>
          Extract Mail
        </button>
        <button disabled={text.length === 0} className={`btn btn-${props.theme} m-2`} onClick={removeClick}>
          Remove Extra Spaces
        </button>
        <button disabled={text.length === 0} className={`btn btn-${props.theme} m-2`} onClick={copyClick}>
          Copy Text
        </button>
        <button disabled={text.length === 0} className={`btn btn-${props.theme} m-2`} onClick={clearClick}>
          Clear Text
        </button>
      </div>
      ...
      <div className="container mb-3 my-3 border p-3">
        ...
        {result.map((data) => {
          ...
          return result.length > 1 ? (
            <li key={`${count++}`} id="myResult" type="1" className="mb-3 my-3">
              {data}
            </li>
          ) : (
            <div id="myResult" className="mb-3 my-3">
              {data}
            </div>
          );
        })}
